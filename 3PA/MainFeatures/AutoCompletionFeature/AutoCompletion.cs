...
namespace _3PA.MainFeatures.AutoCompletionFeature {
    ...
    internal static class AutoCompletion {
        ...
        public static void UpdateAutocompletion(char c = char.MinValue, int insertPosition = -1) {
            ...
            var isNormalContext = SyntaxHighlight.IsCarretInNormalContext(nppCurrentPosition-1); // Subtract one to fix checking for context at end of line.
            ...
            if (c != char.MinValue && !typing && isNormalContext) {
                ...
                if (offset > 0 && strOnLeftLength > offset) {
                    ...
                    if ((hasAtLeastOneLetter || (hasAtLeastOneDigit && !Npp.CurrentFileInfo.IsProgress && !Config.Instance.NppAutoCompleteIgnoreNumbers)) && isNormalContext) { //Added check for context, will prevent casing in single line comments.
